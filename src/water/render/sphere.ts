...
export const lightDirection = d.vec3f(
  0.5773502691896258,
  0.5773502691896258,
  0.5773502691896258,
); // ?
...
export const sphereVertex = tgpu["~unstable"].vertexFn({
  ...
  const viewPosition = std.mul(
    renderLayout.$.uniforms.viewMatrix,
    d.vec4f(realPosition, 1),
  ).xyz;
  const outPosition = std.mul(
    renderLayout.$.uniforms.projectionMatrix,
    d.vec4f(viewPosition.add(corner3), 1),
  );
